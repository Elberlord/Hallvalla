...
function getDragonContractStatusText(def){
  ...
  if(!areDragonContractsUnlocked())return "";
  return "";
}
...
const HALLVALLA_EVENT_UI_STORAGE_KEY="hallvalla_event_ui_settings_v9";
const HALLVALLA_EVENT_UI_LEGACY_STORAGE_KEY="hallvalla_event_ui_settings_v8";
...
const HALLVALLA_HUD_PRESET=Object.freeze({
  ...
  "beast.info.dragonsButton": {
    "x": 269.09674072265625,
    "y": -29.623149871826172,
    "scale": 234,
    "width": 100,
    "height": 100,
    "padding": 0,
...
const HALLVALLA_EVENT_UI_DEFAULTS=Object.freeze({bodySize:17,modalWidth:900,align:"left",selectedHud:"beast.info.actions",hud:{}});
...
function getHallvallaEventUiSettings(){
  try{
    ...
    /*
      Instalación v9: al no existir todavía v9, se aplica una sola vez la
      configuración de fábrica exacta confirmada por el usuario. Después,
      cualquier ajuste posterior se conserva normalmente en localStorage.
    */
    if(!stored){
      ...
      <div class="hallvalla-dragons-overview-artboard">
        ${DRAGON_CONTRACT_ELEMENT_ORDER.map(key=>{
          ...
          return `<div class="hallvalla-dragons-overview-card hallvalla-dragons-overview-card--${key}">
            ...
            <button type="button" class="hallvalla-dragons-overview-openpill" data-dragon-detail-open="${def.id}" aria-label="Abrir evento individual de ${def.enemyName}"></button>
          </div>`;
        }).join('')}
      </div>
    </section>
...
.hallvalla-events-secondary--ghost{min-height:42px;padding:8px 14px;background:rgba(8,10,14,.48);backdrop-filter:blur(4px)}
  ...
  .hallvalla-dragons-overview-card{position:absolute;top:31.5%;width:26%;height:55%;display:flex;flex-direction:column;justify-content:flex-end;gap:8px;padding:0 7.5% 8.8%}
