...
(()=>{
  ...
  const $=id=>document.getElementById(id);
  const TARGETS={
    lobby:{label:'Lobby completo',selector:'#pvpRoomPanel',direct:false},
    art:{label:'Imagen del lobby',selector:'#pvpRoomPanel .pvp-room-art',direct:true},
    codeGroup:{label:'Código + Copiar (grupo)',selector:'#pvpRoomPanel .pvp-room-code-line',direct:false},
    code:{label:'Código',selector:'#pvpRoomCode',direct:true},
    copy:{label:'Botón COPIAR',selector:'#pvpCopyCodeBtn',direct:true},
    player1Group:{label:'Jugador 1 completo',selector:'#pvpRoomPanel .pvp-room-player.player1',direct:false},
    player1Name:{label:'Nombre Jugador 1',selector:'#pvpRoomPlayer1Name',direct:true},
    player1Dot:{label:'Punto conexión J1',selector:'#pvpRoomPlayer1Presence',direct:true},
    player1Check:{label:'Check listo J1',selector:'#pvpRoomPlayer1Check',direct:true},
    player2Group:{label:'Jugador 2 completo',selector:'#pvpRoomPanel .pvp-room-player.player2',direct:false},
    player2Name:{label:'Nombre Jugador 2',selector:'#pvpRoomPlayer2Name',direct:true},
    player2Dot:{label:'Punto conexión J2',selector:'#pvpRoomPlayer2Presence',direct:true},
    player2Check:{label:'Check listo J2',selector:'#pvpRoomPlayer2Check',direct:true},
    ready:{label:'Zona botón LISTO',selector:'#pvpReadyBtn',direct:true},
    leave:{label:'Zona botón SALIR',selector:'#pvpLeaveBtn',direct:true}
  };
  const neutral=()=>({x:0,y:0,scale:100,width:100,height:100});
  const DEFAULTS=Object.fromEntries(Object.keys(TARGETS).map(k=>[k,neutral()]));
  let config=load(), selected='art', shell=null, editor=null, open=false, dragging=null;

  function load(){
    try{
      const raw=JSON.parse(localStorage.getItem(STORAGE_KEY)||'{}');
      const out={};
      for(const key of Object.keys(TARGETS)) out[key]={...neutral(),...(raw[key]||{})};
      return out;
    }catch(_){return structuredClone(DEFAULTS);}
  }
  function save(){try{localStorage.setItem(STORAGE_KEY,JSON.stringify(config));}catch(_){}}
  function el(key){return document.querySelector(TARGETS[key]?.selector||'');}
  function room(){return $('pvpRoomPanel');}
  function roomVisible(){const r=room();return !!(r&&!r.classList.contains('hidden'));}
  function cfg(key){return {...neutral(),...(config[key]||{})};}
  function apply(key){
    const node=el(key); if(!node)return;
    const c=cfg(key);
    node.style.setProperty('--hvLobbyX',`${c.x}px`);
    node.style.setProperty('--hvLobbyY',`${c.y}px`);
    node.style.setProperty('--hvLobbyS',String(c.scale/100));
    node.style.setProperty('--hvLobbySX',String(c.width/100));
    node.style.setProperty('--hvLobbySY',String(c.height/100));
    node.classList.add('hv-lobby-editable');
    node.classList.toggle('hv-lobby-selected',open&&key===selected);
  }
  function applyAll(){Object.keys(TARGETS).forEach(apply);}
  function update(field,value){
    config[selected]={...cfg(selected),[field]:Number(value)};
    save(); apply(selected); sync();
  }
  function nudge(field,delta){update(field,(Number(cfg(selected)[field])||0)+delta);}
  function choose(key){if(!TARGETS[key])return;selected=key;sync();}
  function sync(){
    if(!shell)return;
    const select=$('hvLobbyTarget'); if(select)select.value=selected;
    const name=$('hvLobbySelected'); if(name)name.textContent=TARGETS[selected].label;
    const c=cfg(selected);
    for(const field of ['x','y','scale','width','height']){
      const input=shell.querySelector(`[data-lobby-field="${field}"]`);
      const output=shell.querySelector(`[data-lobby-out="${field}"]`);
      if(input)input.value=String(c[field]);
      if(output)output.textContent=(field==='x'||field==='y')?`${c[field]} px`:`${c[field]}%`;
    }
    applyAll();
  }
  function resetOne(){config[selected]=neutral();save();sync();}
  function resetAll(){config=structuredClone(DEFAULTS);save();sync();}
  async function copyJson(btn){
    const payload=JSON.stringify(config,null,2);
    try{await navigator.clipboard.writeText(payload);const old=btn.textContent;btn.textContent='COPIADO';setTimeout(()=>btn.textContent=old,900);}catch(_){console.info('[HallValla] Lobby layout JSON:',payload);}
  }
  function setOpen(value){
    open=!!value;
    editor?.classList.toggle('hidden',!open);
    const toggle=$('hvLobbyToggle');if(toggle)toggle.textContent=open?'EDITANDO':'EDITAR LOBBY';
    document.body.classList.toggle('hv-lobby-edit-mode',open);
    sync();
  }
  function create(){
    if($('hvLobbyTuner'))return;
    shell=document.createElement('section');
    shell.id='hvLobbyTuner';
    shell.className='hv-lobby-tuner hidden';
    shell.innerHTML=`
      <div class="hv-lobby-tuner-bar">
        <button id="hvLobbyTunerMove" type="button" title="Mover control">⠿</button>
        <button id="hvLobbyToggle" type="button">EDITAR LOBBY</button>
      </div>
      <div id="hvLobbyEditor" class="hv-lobby-tuner-body hidden">
        <div class="hv-lobby-tuner-head"><b>AJUSTAR LOBBY</b><button id="hvLobbyClose" type="button">×</button></div>
        <select id="hvLobbyTarget" class="hv-lobby-target">${Object.entries(TARGETS).map(([key,v])=>`<option value="${key}">${v.label}</option>`).join('')}</select>
        <div id="hvLobbySelected" class="hv-lobby-selected-name"></div>
        <div class="hv-lobby-help">Clic + arrastre: mover solo el elemento · + / −: tamaño · Flechas: ajuste fino.</div>
        ${['x','y','scale','width','height'].map(field=>{
          const label={x:'Horizontal',y:'Vertical',scale:'Tamaño',width:'Ancho',height:'Altura'}[field];
          const min=(field==='x'?-1200:field==='y'?-900:20),max=(field==='x'?1200:field==='y'?900:300);
          return `<div class="hv-lobby-control"><div><b>${label}</b><output data-lobby-out="${field}"></output></div><div class="hv-lobby-control-line"><button type="button" data-lobby-nudge="${field}:-5">−</button><input type="range" data-lobby-field="${field}" min="${min}" max="${max}" step="1"><button type="button" data-lobby-nudge="${field}:5">+</button></div></div>`;
        }).join('')}
        <div class="hv-lobby-actions"><button id="hvLobbyResetOne" type="button">Restaurar</button><button id="hvLobbyResetAll" type="button">Restaurar todo</button><button id="hvLobbyCopyJson" type="button">Copiar JSON</button><button id="hvLobbyDone" type="button">Listo</button></div>
      </div>`;
    document.body.appendChild(shell);
    editor=$('hvLobbyEditor');
    $('hvLobbyToggle').onclick=()=>setOpen(editor.classList.contains('hidden'));
    $('hvLobbyClose').onclick=()=>setOpen(false);
    $('hvLobbyDone').onclick=()=>setOpen(false);
    $('hvLobbyTarget').onchange=e=>choose(e.currentTarget.value);
    shell.querySelectorAll('[data-lobby-field]').forEach(input=>input.addEventListener('input',()=>update(input.dataset.lobbyField,input.value)));
    shell.querySelectorAll('[data-lobby-nudge]').forEach(button=>button.addEventListener('click',()=>{const [field,delta]=button.dataset.lobbyNudge.split(':');nudge(field,Number(delta));}));
    $('hvLobbyResetOne').onclick=resetOne;
    $('hvLobbyResetAll').onclick=resetAll;
    $('hvLobbyCopyJson').onclick=e=>copyJson(e.currentTarget);
    wirePanelDrag(); wireDirectDrag(); wireKeys(); sync(); updateVisibility();
  }
  function wirePanelDrag(){
    const handle=$('hvLobbyTunerMove');if(!handle)return;
    let start=null;
    try{const pos=JSON.parse(localStorage.getItem(PANEL_KEY)||'{}');if(Number.isFinite(pos.left))shell.style.left=`${pos.left}px`;if(Number.isFinite(pos.top))shell.style.top=`${pos.top}px`;}catch(_){ }
    handle.addEventListener('pointerdown',e=>{const rect=shell.getBoundingClientRect();start={px:e.clientX,py:e.clientY,left:rect.left,top:rect.top};handle.setPointerCapture?.(e.pointerId);e.preventDefault();});
    handle.addEventListener('pointermove',e=>{if(!start)return;shell.style.left=`${start.left+e.clientX-start.px}px`;shell.style.top=`${start.top+e.clientY-start.py}px`;});
    const finish=()=>{if(!start)return;start=null;try{localStorage.setItem(PANEL_KEY,JSON.stringify({left:parseFloat(shell.style.left)||16,top:parseFloat(shell.style.top)||100}));}catch(_){}};
    handle.addEventListener('pointerup',finish);handle.addEventListener('pointercancel',finish);
  }
  function wireDirectDrag(){
    document.addEventListener('pointerdown',e=>{
      if(!open||!roomVisible()||shell.contains(e.target))return;
      const entries=Object.entries(TARGETS).filter(([,v])=>v.direct);
      for(const [key] of entries){
        const node=el(key);
        if(node&&(node===e.target||node.contains(e.target))){
          choose(key);const c=cfg(key);dragging={key,pointer:e.pointerId,px:e.clientX,py:e.clientY,x:c.x,y:c.y};node.setPointerCapture?.(e.pointerId);e.preventDefault();return;
        }
      }
    },true);
    document.addEventListener('pointermove',e=>{if(!dragging||!open)return;const c=cfg(dragging.key);c.x=dragging.x+(e.clientX-dragging.px);c.y=dragging.y+(e.clientY-dragging.py);config[dragging.key]=c;save();apply(dragging.key);if(selected===dragging.key)sync();},true);
    const end=()=>{dragging=null;};document.addEventListener('pointerup',end,true);document.addEventListener('pointercancel',end,true);
  }
  function wireKeys(){
    document.addEventListener('keydown',e=>{
      ...
      if(e.key==='+'){nudge('scale',1);e.preventDefault();}
      else if(e.key==='-'){nudge('scale',-1);e.preventDefault();}
      else if(e.key==='ArrowLeft'){nudge('x',-step);e.preventDefault();}
      else if(e.key==='ArrowRight'){nudge('x',step);e.preventDefault();}
      else if(e.key==='ArrowUp'){nudge('y',-step);e.preventDefault();}
      else if(e.key==='ArrowDown'){nudge('y',step);e.preventDefault();}
    });
  }
  function updateVisibility(){
    if(!shell)return;
    const show=roomVisible();
  ...
  function watch(){
    create();
    const r=room();if(r)new MutationObserver(updateVisibility).observe(r,{attributes:true,attributeFilter:['class']});
    const online=$('onlineLobby');if(online)new MutationObserver(updateVisibility).observe(online,{attributes:true,attributeFilter:['class']});
