...
function summonZones(player){const l=getLeader(player);if(!l)return[];const res=[];const rt=typeof isHallvallaRealtimeExperimental==="function"&&isHallvallaRealtimeExperimental();const mid=(ROWS-1)/2;for(let y=0;y<ROWS;y++)for(let x=0;x<COLS;x++){if(getUnitAt(x,y))continue;if(rt){const alliedHalf=Number(player)===1?y>mid:y<mid;if(alliedHalf)res.push(`${x},${y}`);continue;}if(dist(l,{x,y})<=1)res.push(`${x},${y}`)}return res}
...
function getCardPlayState(card){
  ...
  if(card.type==="unit"&&summonZones(myPlayer).length===0)return{canPlay:false,reason:(typeof isHallvallaRealtimeExperimental==="function"&&isHallvallaRealtimeExperimental())?"No hay casillas libres en tu mitad del campo.":"No hay casillas libres junto a tu líder."};
