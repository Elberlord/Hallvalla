...
const UNIT_BATTLE_POWER=Object.freeze({
  ...
  spearman:68,
  archer:51,
  egyptian_line_archer:30,
  new_kingdom_archer:43,
  roman_auxiliary_sagittarius:41,
  arcane_adept:64,
  guardian:71,
  samurai_katana:73,
  samurai_yabusame:67,
...
Object.assign(UNIT_LORE_DATA,{
  ...
  egyptian_line_archer:{short:"Arquero egipcio temprano, ligero y disciplinado, mejor cuando dispara junto a su propia formación.",legend:"Los arqueros egipcios de etapas tempranas combatían con equipo ligero y dependían mucho de la coordinación de grupo. En HallValla esta carta representa esa doctrina: por sí sola es modesta, pero junto a otros arqueros egipcios de línea mejora su puntería y gana consistencia como pequeña formación de hostigamiento."},
  new_kingdom_archer:{short:"Arquero egipcio del Imperio Nuevo, mejor equipado y más capaz de perforar defensas desde una posición preparada.",legend:"Durante el Imperio Nuevo, Egipto empleó arqueros con mejor equipo y mayor profesionalización militar. En HallValla esta unidad premia quedarse quieta y disparar desde una posición preparada: no busca volumen, sino tiros más limpios capaces de castigar mejor la Guardia enemiga."},
  roman_auxiliary_sagittarius:{short:"Arquero auxiliar al servicio de Roma, útil como apoyo disciplinado detrás de la línea de contacto.",legend:"Los sagitarii del ejército romano solían formar parte de unidades auxiliares reclutadas en regiones con tradición arquera. HallValla traduce ese rol como un tirador de apoyo: no depende de duelos aislados, sino de disparar contra enemigos ya comprometidos por un aliado para ganar precisión táctica."},
...
function openUnitLoreModal(entity){
  ...
  const portrait=entity.portrait||entity.cardImage||entity.heroImage||"";
...
function applyBurnAtTurnEnd(units){
  ...
  let out=(units||[]).map(u=>{
    ...
    let next=resolveBlessedArmorTransition(u,{...u,hp:(u.hp||0)-dmg,burnTurns:turnsBefore-1,damagedThisTurn:true});
...
function countAdjacentAlliesByKey(units,unit,key){
  if(!unit||!Array.isArray(units))return 0;
  const ux=Number(unit.x),uy=Number(unit.y);
  return units.filter(other=>other&&other.id!==unit.id&&other.owner===unit.owner&&other.hp>0&&String(other.key||"").toLowerCase()===String(key||"").toLowerCase()&&Math.abs(Number(other.x)-ux)+Math.abs(Number(other.y)-uy)<=1).length;
}
function targetHasAnotherAdjacentAlly(attacker,defender,units){
  if(!attacker||!defender||!Array.isArray(units))return false;
  const dx=Number(defender.x),dy=Number(defender.y);
  return units.some(other=>other&&other.id!==attacker.id&&other.owner===attacker.owner&&other.hp>0&&Math.abs(Number(other.x)-dx)+Math.abs(Number(other.y)-dy)<=1);
}
function applyHistoricalArcherCombatMods(attacker,defender,units,mods){
  let next={...(mods||{})};
  const logs=[];
  if(!attacker||!defender)return {mods:next,text:""};
  const ranged=isRangedAttack(attacker,defender);
  if(!ranged)return {mods:next,text:""};
  if(attacker.key==="egyptian_line_archer"){
    const bonus=Math.min(2,countAdjacentAlliesByKey(units,attacker,"egyptian_line_archer"));
    if(bonus>0){next.attackerDex=(next.attackerDex||0)+bonus;logs.push(`Descarga coordinada: +${bonus} DX.`);}
  }
  if(attacker.key==="new_kingdom_archer"){
    const movedSpaces=Math.max(0,Number(attacker.movedSpaces||0));
    if(!attacker.moved&&movedSpaces===0){next.defenderGuard=(next.defenderGuard||0)-2;logs.push("Tiro preparado: el objetivo combate con -2 Guardia.");}
  }
  if(attacker.key==="roman_auxiliary_sagittarius"&&targetHasAnotherAdjacentAlly(attacker,defender,units)){
    next.attackerDex=(next.attackerDex||0)+2;logs.push("Cobertura auxiliar: +2 DX.");
  }
  return {mods:next,text:logs.length?` ${logs.join(" ")}`:""};
}
...
function setHint(t){setText("hint",t)}function isBattleEnded(){return !!(publicState?.phase==="ended"||publicState?.battleEnded)}async function pushLog(t){if(!gameId||!publicState||isTurnWriteBlockedByExpiredClock())return;const logs=[t,...(publicState.log||[])].slice(0,18);if(hallvallaIsLocalTestGame()){publicState={...publicState,log:logs};render();maybeStartTurn();maybeTriggerAdventureAI();return;}await update(ref(db,`games/${gameId}/public`),{log:logs})}async function updatePublic(patch){
...
function showCardInspectModal(card){
  ...
  if(text){
    ...
    text.innerHTML=`${renderDetAbilitiesHtml(card,effectText)}${renderDetStatusesHtml([],card)}${renderDetQuoteHtml(card)}${detailGuideButtonsHtml({showEffect:shouldShowEffectGuideButton(card,effectText),showWeapon:true,showFormula:true,showLore:card.type==='unit',effectLabel:'Ver efecto de la carta',entity:card})}`;
...
async function attackUnit(a,d){
  ...
  const historicalArcherPrep=applyHistoricalArcherCombatMods(a,d,units,mods);
  mods=historicalArcherPrep.mods;
  const historicalArcherText=historicalArcherPrep.text;
  let firstStrikeText="";
  ...
  const canSpecialCounter=defenderAfter&&attackerAfter&&!mods.noCounter&&!defenderAfter.counterUsedTurn&&arcaneAdeptRangedCounter;
...
async function adventureEnemyTurn(){
  ...
  const attackWith=async(attacker)=>{
    ...
    const historicalArcherPrep=applyHistoricalArcherCombatMods(attacker,target,units,mods);
    mods=historicalArcherPrep.mods;
    const historicalArcherText=historicalArcherPrep.text;
    ...
    const ninjutsuExtraText=`${geishaFanKillResult?.text||""}${saboteadorEscapeResult?.text||""}${stealthText}${hanzoContractResult.text||""}${simoStealthResult.text||""}`;
    const vikingExtraText=`${ulfhednarCritResult.text||""}${berserkerOsoText}${skiparWarLootText}`;
    const actionLog=hit.hit?`Rival: ${attacker.name} ataca a ${target.name}: acierta (${hit.roll}/${hit.chance}).${rerollText}${combatSummary(mods)}${warningRune.text||""}${historicalArcherText}${assassinIgnoreText} ${guardLoss>0?`Consume ${guardLoss} GD de este turno. `:""}${hpLoss>0?`Inflige ${hpLoss} daño a HP.`:"No atraviesa la guardia."}${vikingExtraText}${pressureText}${actionSpendText}${warCryText}${bloodVictoryText}${leonidasLastStandText}${bloodMistText}${steelWallText}${coverFireText}${alexanderWallText}${ulyssesTacticText}${bloodBaitText}${genghisDebuffText}${bleedText}${falconRecoilText}${porcupineText}${lionFearText}${rhinoStunText}${elephantChargeText}${warriorShieldText}${counterText}${mulanExecutionText}${khalidChainText}${masteryKillText}${samuraiExtraText}${ninjutsuExtraText}`:`Rival: ${attacker.name} ataca a ${target.name}: falla (${hit.roll}/${hit.chance}).${rerollText}${combatSummary(mods)}${warningRune.text||""}${historicalArcherText}${pressureText}${actionSpendText}${alexanderWallText}${ulyssesTacticText}${porcupineText}${lionFearText}${elephantChargeText}${counterText}${samuraiExtraText}${ninjutsuExtraText}`;
  ...
  const moveUnitSmart=(u)=>{
    ...
    units=trapMove.cancel?trapMove.units:trapMove.units.map(it=>it.id===u.id?{...it,x:best.x,y:best.y,moved:true,movedSpaces:(it.movedSpaces||0)+movedNow,lastMoveStraightDistance:straightMoveNow,lastMoveDistance:movedNow,lastMoveDx:Math.sign(best.x-u.x),lastMoveDy:Math.sign(best.y-u.y),lastMoveTurnKey:publicState?.turnKey||""}:it);
  ...
  const playDamageSpell=(choice)=>{
    ...
    units=units.map(u=>{
      ...
      if(appliesBurn&&damagedTarget.hp>0)damagedTarget=applyBurnToUnit(damagedTarget,choice.card.name,choice.card.burnTurns||2,choice.card.burnDamage||1);
      if(appliesSandSlow&&damagedTarget.hp>0)damagedTarget={...damagedTarget,mov:Math.max(0,Number(damagedTarget.mov||0)-sandSlowAmount)};
      return damagedTarget;
    });
    if(appliesBurn&&damagedTarget&&damagedTarget.hp>0)pendingAiStatusFxEvent=makeStatusFxEvent("burn_apply",damagedTarget,1);
    ...
    else if(appliesSandSlow&&damagedTarget&&damagedTarget.hp>0)pendingAiStatusFxEvent=makeStatusFxEvent("debuff",damagedTarget,sandSlowAmount);
    honor-=effectiveCardCost(choice.card,2);
    removeCard(choice.card);
    logs.push(`Rival usa ${choice.card.name}: ${originalTarget.name} recibe ${dmg} daño${originalTarget.key==="honey_badger"?" tras Armadura Natural":""}${appliesBurn?" y queda con Quemadura: +1 daño directo al final de cada turno durante 2 turnos":""}${appliesSandSlow?` y pierde -${sandSlowAmount} MOV permanente`:""}.`);
    killDead();
    return true;
  };
  ...
  const playGuard=(choice)=>{
    ...
    honor-=effectiveCardCost(choice.card,2);
    removeCard(choice.card);
    logs.push(bhTrap.cancel?bhTrap.logs.join(" "):(choice.card.trap==="guard"?`Rival coloca ${choice.card.name} sobre ${choice.ally.name}. La próxima vez que sea atacada obtendrá +${choice.card.guard||0} GUARDIA durante ese combate.`:`Rival usa ${choice.card.name}: ${choice.ally.name} gana +${choice.card.guard||0} GUARDIA durante 2 turnos (turno actual y próximo turno rival).`));
    return true;
  };
  ...
  const playHeal=(choice)=>{
    ...
    honor-=effectiveCardCost(choice.card,2);
    removeCard(choice.card);
    logs.push(bhTrap.cancel?bhTrap.logs.join(" "):`Rival usa ${choice.card.name}: ${choice.ally.name} cura ${healAmount} HP${canCleanse&&hasCurableStatus(choice.ally)?" y limpia estados curables":""}.`);
    return true;
  };
