...
function applyLanceWeaponRule(card){
  ...
  const firstStrikeText=" Regla de lanza — Atacar primero: cuando una unidad enemiga de cuerpo a cuerpo con RG 1 la ataca desde una casilla adyacente, ataca primero una vez por turno. No funciona contra unidades con RG 2 o más ni contra Ataque en Picada del halcón.";
...
function applyDemigodLanceFirstStrikeText(card){
  ...
  const ruleText=" Regla de semidiós lancero: como unidad de lanza, ataca primero una vez por turno únicamente cuando una unidad enemiga de cuerpo a cuerpo con RG 1 lo ataca desde una casilla adyacente; si derrota al atacante, cancela ese ataque.";
  const current=String(card.text||card.effectText||card.ability||"");
...
function canLanceFirstStrike(attacker,defender,mods={}){
  ...
  if(defender.counterUsedTurn)return false;
...
function resolveLanceFirstStrike(attacker,defender,units){
  ...
  units=(units||[]).map(u=>u.id===currentDefender.id?{...u,counterUsedTurn:true}:u);
  currentAttacker=(units||[]).find(u=>u.id===currentAttacker.id)||currentAttacker;
  currentDefender=(units||[]).find(u=>u.id===currentDefender.id)||currentDefender;
...
function setHint(t){setText("hint",t)}function isBattleEnded(){return !!(publicState?.phase==="ended"||publicState?.battleEnded)}async function pushLog(t){if(!gameId||!publicState)return;const logs=[t,...(publicState.log||[])].slice(0,18);if(hallvallaIsLocalTestGame()){publicState={...publicState,log:logs};render();maybeStartTurn();maybeTriggerAdventureAI();return;}await update(ref(db,`games/${gameId}/public`),{log:logs})}async function updatePublic(patch){if(hallvallaIsLocalTestGame()){const prevPublic=publicState?JSON.parse(JSON.stringify(publicState)):null;publicState=hallvallaApplyLocalPatch(publicState,patch);render();syncBattleMusic();maybePlayBattleFx(prevPublic,publicState);maybeShowBattleResult();maybeStartTurn();maybeTriggerAdventureAI();return;}await update(ref(db,`games/${gameId}/public`),patch)}async function updatePrivate(patch){if(hallvallaIsLocalTestGame()){privateState=hallvallaApplyLocalPatch(privateState,patch);render();maybeStartTurn();maybeTriggerAdventureAI();return;}await update(ref(db,`games/${gameId}/private/player${myPlayer}`),patch)}async function updateUnits(units){await updatePublic({units})}function hasLivingNonLeaderUnitsForOwner(owner,units=publicState?.units||[]){
...
function weaponGuideData(entity){
  if(entity&&!entity.spell&&!entity.trap&&!entity.leader){
    ...
    const extra=isLanceUnitCardLike(entity)
      ?" Regla de lanza: si una unidad enemiga de cuerpo a cuerpo con RG 1 la ataca desde una casilla adyacente, ataca primero una vez por turno. Las unidades con RG 2 o más no activan esta reacción. Contra Caballería conserva +5 DX y Anticaballería si el combate es cuerpo a cuerpo."
      :"";
    return {
  ...
  if(key==="spearman"||name.includes("lancero"))return {title:"Lanza y escudo",short:"Arma defensiva de control que castiga entradas cuerpo a cuerpo.",formula:"Ventaja: puede atacar primero una vez por turno cuando una unidad enemiga con RG 1 lo ataca desde una casilla adyacente. No reacciona contra arqueras ni otras unidades con RG 2 o más. Es especialmente peligroso contra Caballería en combate cuerpo a cuerpo.",example:"Colócalo en el frente para bloquear unidades cuerpo a cuerpo; los ataques de rango pueden hostigarlo sin activar Formación de picas."};
...
function getExactEffectGuideDataLegacy7haj(entity,effectText=""){
  ...
  if(isUnit){
    ...
    if(isLanceUnitCardLike(entity))systemLines.push("Regla global de lanza: ataca primero una vez por turno solo contra una unidad enemiga de cuerpo a cuerpo con RG 1 que ataque desde una casilla adyacente. No responde contra unidades con RG 2 o más. Anticaballería también es innata: en combate cuerpo a cuerpo, atacando o defendiendo, la Caballería rival queda con Guardia 0 y AGI 0 durante ese combate.");
...
• Una vez por turno, cuando una unidad enemiga de cuerpo a cuerpo con RG 1 lo ataca desde una casilla adyacente, el Lancero Solar ataca primero. No se activa contra arqueras ni otras unidades con RG 2 o más.
...
async function adventureEnemyTurn(){
  ...
  const playDamageSpell=(choice)=>{
    ...
    honor-=effectiveCardCost(choice.card,2);
    removeCard(choice.card);
    logs.push(`Rival usa ${choice.card.name}: ${originalTarget.name} recibe ${dmg} daño${originalTarget.key==="honey_badger"?" tras Armadura Natural":""}${appliesBurn?" y queda con Quemadura: +1 daño directo al final de cada turno durante 2 turnos":""}.`);
    killDead();
    return true;
  };
  ...
  const playGuard=(choice)=>{
    ...
    units=bhTrap.cancel?bhTrap.units:units.map(u=>u.id===choice.ally.id?{...u,tempGuardBuff:(u.tempGuardBuff||0)+effectiveCardValue(choice.card,"guard")}:u);
    legendaryTraps=bhTrap.traps;
    honor-=effectiveCardCost(choice.card,2);
    removeCard(choice.card);
    logs.push(bhTrap.cancel?bhTrap.logs.join(" "):`Rival usa ${choice.card.name}: ${choice.ally.name} gana +${choice.card.guard||0} GUARDIA temporal.`);
    return true;
  };
...
function getCodeTruthGlobalRuleLines7hai(entity){
  ...
  if(isLanceUnitCardLike(entity))lines.push(`Regla global de lanza: ataca primero una vez por turno únicamente contra una unidad enemiga de cuerpo a cuerpo con RG 1 que ataque desde una casilla adyacente. Las unidades con RG 2 o más no activan esta reacción. Si derrota al atacante, cancela ese ataque. Halcón con Ataque en Picada también la ignora. Anticaballería es igualmente innata: en combate cuerpo a cuerpo, atacando o defendiendo, la Caballería rival queda con Guardia 0 y AGI 0 durante ese combate.`);
...
function getExactEffectGuideData(entity,effectText=""){
  ...
  const displayRaw=raw
    .replace(/\s*Regla de lanza: (?:puede contraatacar una vez por turno si sobrevive|ataca primero una vez por turno únicamente contra una unidad enemiga de cuerpo a cuerpo con RG 1 que ataque desde una casilla adyacente)\./gi,"")
