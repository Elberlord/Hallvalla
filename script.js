...
-------------------------------------------------------------------------------
*/
const HALLVALLA_BUILD_VERSION="v8_LOCAL_RARITY_REAL_BOARD_7HDJ";
...
// Todas las unidades que usan lanza/alabarda/pica pueden atacar primero una vez por turno cuando son atacadas dentro de su alcance de reacción.
...
function applyLanceWeaponRule(card){
  ...
  // Balance 7HCV: las lanzas conservan su rango propio y ganan Atacar Primero.
  // Su alcance de reacción se resuelve por separado en getCounterRange.
  card.range=Math.max(1,card.range||1);
  const ruleText=" Regla de lanza: cuando una unidad enemiga la ataca dentro de su alcance, ataca primero una vez por turno. No funciona contra Ataque en Picada del halcón.";
...
Object.assign(UNIT_LORE_DATA,{
  ...
  spearman:{short:"Infantería de picas y lanza, defensa solar contra cargas y amenazas frontales.",legend:"El Lancero solar representa a la línea disciplinada que sostiene el campo con formación y alcance de lanza. En HallValla castiga especialmente a la caballería y puede responder si sobrevive al ataque."},
...
// La regla real ahora es general para todas las unidades de lanza: Atacar Primero.
...
function applyDemigodLanceFirstStrikeText(card){
  ...
  const ruleText=" Regla de semidiós lancero: como unidad de lanza, cuando es atacado dentro de su alcance ataca primero una vez por turno; si derrota al atacante, cancela ese ataque.";
  const current=String(card.text||card.effectText||card.ability||"");
...
function weaponGuideData(entity){
  if(entity&&!entity.spell&&!entity.trap&&!entity.leader){
    ...
    const extra=isLanceUnitCardLike(entity)
      ?" Regla de lanza: si una unidad enemiga la ataca dentro de su alcance, ataca primero una vez por turno. Contra Caballería conserva +5 DX y Anticaballería si el combate es cuerpo a cuerpo."
      :"";
    return {
...
function showCardInspectModal(card){
  ...
  if(text){
    ...
    text.innerHTML=`${renderDetAbilitiesHtml(card,effectText)}${renderDetTacticalHtml(card)}${renderDetStatusesHtml([],card)}${renderDetQuoteHtml(card)}${detailGuideButtonsHtml({showEffect:shouldShowEffectGuideButton(card,effectText),showWeapon:true,showFormula:true,showLore:card.type==='unit',effectLabel:'Ver efecto de la carta',entity:card})}`;
...
function showUnit(u){
  ...
  inspectTextEl.innerHTML=`${renderDetAbilitiesHtml(u,fx)}${renderDetTacticalHtml(u)}${renderDetStatusesHtml(activeEntries,u)}${renderDetQuoteHtml(u)}${detailGuideButtonsHtml({showEffect:shouldShowEffectGuideButton(u,fx),showWeapon:true,showFormula:true,showLore:true,effectLabel:u.leader?'Ver líder':'Ver efecto',entity:u})}`;
...
function getCodeTruthGlobalRuleLines7hai(entity){
  ...
  if(isLanceUnitCardLike(entity))lines.push(`Regla global de lanza: ataca primero una vez por turno cuando un enemigo la ataca dentro de su alcance. Si derrota al atacante, cancela ese ataque. Halcón con Ataque en Picada ignora esta reacción.`);
...
const CODE_TRUTH_EFFECTS_7HAI={
  ...
  berserker:{trigger:["Regla de hacha activa siempre: +2 DX base ya incluida en DET.","Ruptura brutal se activa al declarar ataque cuerpo a cuerpo."],does:["El objetivo recibe -3 Guardia durante ese combate.","Si el ataque causa daño, primero baja Guardia y el sobrante baja Vida."],doesNot:["Ruptura brutal no es daño directo: solo reduce Guardia para ese combate.","Si lo ataca un Lancero solar, el Lancero no aplica penalización propia por no ser Caballería, pero puede responder por regla de lanza si sobrevive."],example:"Berserker vs Lancero: Berserker reduce Guardia del Lancero; si el Lancero sobrevive, puede contraatacar por regla de lanza."},
  spearman:{trigger:["Anticaballería se activa si el atacante cuenta como Caballería y lo ataca cuerpo a cuerpo.","La respuesta normal de lanza se rige por la regla global de lanzas."],does:["Contra Caballería: el atacante queda con AGI 0 y Guardia 0 durante ese combate.","Si sobrevive y el ataque estuvo dentro de su rango de contraataque, puede contraatacar por regla de lanza."],doesNot:["No aplica penalización propia contra unidades que no sean Caballería.","No contraataca si no sobrevive, si el ataque no estuvo dentro de su rango de respuesta o si ya gastó ese contraataque."],example:"Contra no Caballería no agrega castigo especial; solo conserva la respuesta normal de lanza si sobrevive."},
...
function getExactEffectGuideData(entity,effectText=""){
  ...
  const displayRaw=raw
    .replace(/\s*Regla de lanza: puede contraatacar una vez por turno si sobrevive\./gi,"")
