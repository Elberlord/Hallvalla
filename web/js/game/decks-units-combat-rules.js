...
function setHint(t){setText("hint",t)}function isBattleEnded(){return !!(publicState?.phase==="ended"||publicState?.battleEnded)}async function pushLog(t){if(!gameId||!publicState)return;const safeText=typeof sanitizeSharedStealthText==="function"?sanitizeSharedStealthText(t,publicState.units||[]):t;const previousLogs=(publicState.log||[]).map(line=>typeof sanitizeSharedStealthText==="function"?sanitizeSharedStealthText(line,publicState.units||[]):line);const logs=[safeText,...previousLogs].slice(0,18);if(globalThis.hallvallaRtUseLocalBattleRuntime?.()){publicState={...publicState,log:logs};networkPublicStateRaw=publicState?JSON.parse(JSON.stringify(publicState)):networkPublicStateRaw;render();globalThis.hallvallaRtScheduleLocalSnapshot?.(false);return;}if(hallvallaIsLocalTestGame()){publicState={...publicState,log:logs};render();return;}await update(ref(db,`games/${gameId}/public`),{log:logs})}
