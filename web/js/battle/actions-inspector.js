...
function canUnitDeclareAttack(u){
  ...
  if((live.noAttackTurnKey&&live.noAttackTurnKey===publicState?.turnKey)||isRtTrapLocked(live,"attack"))return false;
  return true;
}
...
function isDirectTacticalUnitSelection(){
  if(selectedCard||selectedUnitActionMode||!selectedUnitId||!publicState||isBattleEnded())return false;
  const u=getUnit(selectedUnitId);
  return !!(u&&u.owner===myPlayer&&isMyTurn()&&isActionPhase());
}
...
function moveZones(u){
  ...
  if(!mulanExecMove&&((live.noMoveTurnKey&&live.noMoveTurnKey===publicState?.turnKey)||isRtTrapLocked(live,"move")))return[];
...
function getTacticalPreviewClasses(x,y){
  ...
  // Lectura de amenaza enemiga: al consultar una unidad rival se conserva
  // la previsualización completa de su alcance teórico para planificación.
  if(u.owner!==myPlayer){
  ...
  // CONTROL DIRECTO 38: al seleccionar una unidad propia, verde significa
  // destino MOV realmente ejecutable y rojo SOLO marca enemigos atacables ahora.
  const moveSet=new Set(moveZones(u));
  const attackSet=new Set(getAttackableTargets(u,publicState?.units||[]).map(t=>`${t.x},${t.y}`));
  if(attackSet.has(key))return["attack-range-preview"];
  if(moveSet.has(key))return["move-range-preview"];
  return[];
}
function clearSelection(){selectedCard=null;selectedUnitId=null;selectedUnitActionMode=null;selectedUnitEffectChoice=null;cardInspectSelection=null;unitContextSelection=null;hideUnitContextMenu();hideCardInspectModal();highlights=[];highlightType="move";dragMoveHighlights=[];dragAttackHighlights=[];dragSummonHighlights=[];if(typeof hallvallaRtReleaseHandFocus==="function")hallvallaRtReleaseHandFocus();render()}
...
function getCardPlayState(card){
  ...
  if(!(typeof isHallvallaRealtimeExperimental==="function"&&isHallvallaRealtimeExperimental())&&!isMyTurn())return{canPlay:false,reason:"No es tu turno."};
  if(!(typeof isHallvallaRealtimeExperimental==="function"&&isHallvallaRealtimeExperimental())&&!isHandPlayPhase())return{canPlay:false,reason:`Solo puedes jugar cartas desde la mano en Main Phase o Last Phase. Fase actual: ${turnPhaseLabel()}.`};
...
function getPlayableCardsInHand(){
  ...
  if(!publicState||!privateState||(!(typeof isHallvallaRealtimeExperimental==="function"&&isHallvallaRealtimeExperimental())&&!isMyTurn())||isBattleEnded())return[];
...
function canPlayCardWithSnapshot(card,honor,phase,units,player){
  ...
  if(card.spell==="damage")return unitsList.some(u=>u.owner!==player&&canDirectlyTarget(card,u))||canDamageSpellAffectEnemyUndeadRemains(card,player,publicState);
  if(card.spell==="paralysis"||card.spell==="poison")return unitsList.some(u=>u.owner!==player&&!u.leader&&canDirectlyTarget(card,u)&&(card.spell!=="poison"||!isPoisonImmuneUnit(u)));
...
/* v216 · eliminado: auto-avance Main/Action/Last/End del motor por turnos. */
...
function syncHandAutoClose(){
  ...
  if(typeof isHallvallaRealtimeExperimental==="function"&&isHallvallaRealtimeExperimental()){const suppressed=typeof hallvallaRtIsHandSuppressed==="function"&&hallvallaRtIsHandSuppressed();handOpen=!suppressed;handManualCloseKey="";return;}
...
function closeHandForBoardFocus(){
  if(typeof isHallvallaRealtimeExperimental==="function"&&isHallvallaRealtimeExperimental()){if(typeof hallvallaRtSuppressHandFocus==="function")hallvallaRtSuppressHandFocus();handOpen=false;const drawer=$("handDrawer");if(drawer)drawer.classList.remove("open");return;}
  handOpen=false;
