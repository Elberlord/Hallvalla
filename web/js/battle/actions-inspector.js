...
function isPvpStep6fLimitedMode(){try{return globalThis.__HALLVALLA_PVP_STEP6F_LIMITED__?.()===true}catch(_){return false}}
...
function getUnitAttackRange(u){
  ...
  let range=baseRange+Number(bonus.range||0)+Number(arcaneLink.range||0)+Number(getEquipmentRangeBonus(u)||0)-Number(u?.tempRangeDebuff||0)-getRtTrapDebuff(u,"range");
...
function getTacticalPreviewClasses(x,y){
  ...
  if(selectedCard||!unitContextSelection||!publicState)return[];
  const u=getUnit(unitContextSelection.unitId);
  if(!u||u.owner===myPlayer)return[];
  if(typeof isStealthHiddenFromViewer==="function"&&isStealthHiddenFromViewer(u))return[];
  const key=`${x},${y}`;
  const threatSet=new Set(attackRangeCells(u,publicState?.units||[]));
  if(!threatSet.has(key))return[];
  return [u.acted?"enemy-acted-threat-preview":"enemy-threat-preview"];
}
function clearSelection(){selectedCard=null;selectedUnitId=null;cardInspectSelection=null;unitContextSelection=null;hideUnitContextMenu();hideCardInspectModal();highlights=[];highlightType="move";dragSummonHighlights=[];if(typeof hallvallaRtReleaseHandFocus==="function")hallvallaRtReleaseHandFocus();render()}
function canDamageSpellAffectEnemyUndeadRemains(card,player=myPlayer,state=publicState){
  if(!card||card.spell!=="damage")return false;
  const kind=getCardMagicDamageType(card);
  if(kind!=="fire"&&kind!=="ice")return false;
  return getUndeadRemains(state).some(r=>Number(r.owner)!==Number(player));
}
function getEnemyUndeadRemainsCellsForCard(card,player=myPlayer,state=publicState){
  if(!canDamageSpellAffectEnemyUndeadRemains(card,player,state))return[];
  return getUndeadRemains(state).filter(r=>Number(r.owner)!==Number(player)).map(r=>`${r.x},${r.y}`);
}
function getCardPlayState(card){
  ...
  if(isPvpStep6fLimitedMode()&&!isPvpStep6hAllowedHandCard(card))return{canPlay:false,reason:"Paso 6H: por ahora están habilitadas las cartas de UNIDAD y Fireball. Otros hechizos, equipos y trampas siguen bloqueados."};
  ...
  if(card.spell==="damage"&&!(publicState.units||[]).some(u=>u.owner!==myPlayer&&canDirectlyTarget(card,u))&&!canDamageSpellAffectEnemyUndeadRemains(card,myPlayer,publicState))return{canPlay:false,reason:"No hay unidades ni Restos Persistentes rivales válidos para este hechizo."};
  if((card.spell==="paralysis"||card.spell==="poison")&&!(publicState.units||[]).some(u=>u.owner!==myPlayer&&!u.leader&&canDirectlyTarget(card,u)&&(card.spell!=="poison"||!isPoisonImmuneUnit(u))))return{canPlay:false,reason:"No hay invocaciones rivales válidas para este hechizo."};
...
function getPlayableCardsInHand(){
  ...
  if(!publicState||!privateState||isBattleEnded())return[];
  return hand.filter(c=>getCardPlayState(c).canPlay);
}

function syncHandAutoClose(){
  if(!publicState||!privateState)return;
  const suppressed=typeof hallvallaRtIsHandSuppressed==="function"&&hallvallaRtIsHandSuppressed();
  handOpen=!suppressed;
  handManualCloseKey="";
}
...
function weaponGuideData(entity){
  if(entity&&!entity.spell&&!entity.trap&&!entity.leader){
    ...
    const extra=isLanceUnitCardLike(entity)
      ?" Regla de lanza: tiene RG 1 fijo. La primera vez cada 10 s que una unidad enemiga de cuerpo a cuerpo con RG 1 la ataca desde una casilla adyacente, ataca antes que ella. Las unidades con RG 2 o más no activan esta reacción. Contra Caballería conserva +5 DX y Anticaballería si el combate es cuerpo a cuerpo."
      :"";
    return {
  ...
  if(key==="spearman"||name.includes("lancero"))return {title:"Lanza y escudo",short:"Arma defensiva de control que castiga entradas cuerpo a cuerpo.",formula:"Ventaja: tiene RG 1 fijo. La primera vez cada 10 s que una unidad enemiga con RG 1 lo ataca cuerpo a cuerpo desde una casilla adyacente, ataca antes que ella. No reacciona contra arqueras ni otras unidades con RG 2 o más. Es especialmente peligroso contra Caballería en combate cuerpo a cuerpo.",example:"Colócalo en el frente para bloquear unidades cuerpo a cuerpo; los ataques de rango pueden hostigarlo sin activar Formación de picas."};
  if(key==="archer"||name.includes("arquera")||name.includes("arquero"))return {title:"Arco",short:"Arma de hostigamiento. Hace daño desde distancia y obliga al rival a moverse mal.",formula:"Ventaja: ataca fuera del cuerpo a cuerpo. Además, su disparo puede reducir MOV del objetivo, cortando persecuciones o retiradas.",example:"Una arquera bien colocada gana valor si dispara sin quedar atrapada después del ataque."};
  ...
  if(range>=3)return {title:"Arma a distancia",short:"Ataca desde lejos. Su valor está en pegar sin recibir contraataque inmediato.",formula:"Ventaja: mientras mantenga distancia, puede forzar al rival a gastar movimiento antes de responder. DX y AGI definen qué tan confiable será el disparo.",example:"Protege esta unidad con frontales para que pueda mantener fuego a distancia durante más tiempo."};
  ...
  if(guard>=5)return {title:"Arma defensiva / escudo",short:"Su equipo está pensado para aguantar más que para borrar enemigos rápido.",formula:"Ventaja: absorbe daño, protege casillas y compra tiempo. Mientras sobreviva, su Guardia puede restaurarse periódicamente.",example:"Excelente para formar pared delante de arqueros o líderes."};
...
function statGuideData(label=""){
  ...
  const map={
    ...
    gd:{title:"🛡 GD / Guardia",short:"Armadura temporal. Amortigua daño y se restaura periódicamente si la unidad sobrevive.",formula:"Daño a Vida = Daño recibido - Guardia disponible. La Guardia consumida baja al recibir daño y se restaura periódicamente a su valor base más buffs activos.",example:"Si recibes 4 de daño con 3 GD, pierdes 3 GD y solo 1 HP. Si sobrevives, tu GD vuelve en la siguiente restauración periódica."},
    guardia:{title:"🛡 GD / Guardia",short:"Armadura temporal. Amortigua daño y se restaura periódicamente si la unidad sobrevive.",formula:"Daño a Vida = Daño recibido - Guardia disponible. La Guardia consumida baja al recibir daño y se restaura periódicamente a su valor base más buffs activos.",example:"Si recibes 4 de daño con 3 GD, pierdes 3 GD y solo 1 HP. Si sobrevives, tu GD vuelve en la siguiente restauración periódica."},
    dx:{title:"⌁ DX / Destreza",short:"Dominio del combate: técnica, coordinación, experiencia, lectura, timing, táctica y astucia. Se combina con AGI para atacar y defender.",formula:"DX se suma con AGI para crear la reserva PREC/EVA disponible. Al atacar, el golpe primero presiona/consume EVA del defensor y luego se compara contra la EVA restante. Al atacar o recibir presión, la reserva gastada vuelve en la siguiente restauración periódica.",example:"Una unidad técnica puede conservar defensa si ataca a objetivos con poca evasión, porque solo gasta lo necesario."},
    destreza:{title:"⌁ DX / Destreza",short:"Dominio del combate: técnica, coordinación, experiencia, lectura, timing, táctica y astucia. Se combina con AGI para atacar y defender.",formula:"DX se suma con AGI para crear la reserva PREC/EVA disponible. Al atacar, el golpe primero presiona/consume EVA del defensor y luego se compara contra la EVA restante. Al atacar o recibir presión, la reserva gastada vuelve en la siguiente restauración periódica.",example:"Una unidad técnica puede conservar defensa si ataca a objetivos con poca evasión, porque solo gasta lo necesario."},
    agi:{title:"➤ AGI / Agilidad",short:"Velocidad. Suma tanto para conectar ataques como para esquivarlos.",formula:"AGI se suma con DX para crear la reserva PREC/EVA disponible. Al atacar, el golpe primero presiona/consume EVA del defensor y luego se compara contra la EVA restante. Atacar y recibir presión gastan reserva; se restaura periódicamente.",example:"Un asesino ágil puede atacar y aún conservar defensa si el objetivo exigía poca evasión."},
...
function closeHandForBoardFocus(){
  if(typeof hallvallaRtSuppressHandFocus==="function")hallvallaRtSuppressHandFocus();
  handOpen=false;
  handManualCloseKey="";
...
function ensureUnifiedDetNameCalibration(modal,entity){
  ...
  box.dataset.detSurface='name';
  box.setAttribute('aria-label','Nombre de la carta');
  const value=typeof getEntityFullDisplayName==='function'?getEntityFullDisplayName(entity):String(entity?.name||'Unidad');
  const text=document.createElement('div');
  text.className='hv-det-name-text';
  text.textContent=value||'Unidad';
  const id=document.createElement('span');
  ...
  set('text-shadow','0 2px 3px rgba(0,0,0,.95),0 0 7px rgba(210,151,45,.20)');
  set('transform-origin','center');
  set('pointer-events','auto',false);
  text.style.setProperty('display','block','important');
  text.style.setProperty('visibility','visible','important');
  text.style.setProperty('opacity','1','important');
  text.style.setProperty('color','#f2d28a','important');
...
function ensureUnifiedDetActiveEffects(modal,statuses=[]){
  ...
  entries.forEach((entry,index)=>{
    ...
    const desc=typeof hallvallaPublicGameplayText==='function'?hallvallaPublicGameplayText(entry?.desc||''):(entry?.desc||'');
...
function openUnifiedDetEntity(entity,{mode="card",live=false,statuses=[],visualHtml="",allowPlay=false,playState=null}={}){
  ...
  cleanupUnifiedDetSurface(modal);
...
function clearCurableStatuses(u){
  ...
  delete next.bleedCyclesRemaining;
  delete next.poisonDamage;
  delete next.poisonTurns;
  delete next.poisonStage;
  delete next.poisonBaseDamage;
  delete next.poisonMaxDamage;
  delete next.poisonPersistent;
  delete next.poisonSourceId;
  delete next.poisonSourceName;
  delete next.noHealWhilePoisoned;
  delete next.burnTurns;
  delete next.burnDamage;
  delete next.burnPersistent;
  delete next.burnSourceName;
  return next;
}
function hasCurableStatus(u){
  if(!u)return false;
  const poisoned=(Number(u.poisonTurns||0)>0||u.poisonPersistent===true)&&Number(u.poisonDamage||0)>0&&!u.noHealWhilePoisoned;
  const burning=(Number(u.burnTurns||0)>0||u.burnPersistent===true)&&Number(u.burnDamage||0)>0;
  return hasBleeding(u)||poisoned||burning;
}
function canHealOrCleanseUnit(u,owner=null){
  if(!u)return false;
  if(owner!==null&&u.owner!==owner)return false;
  if((u.noHealWindowKey&&u.noHealWindowKey===publicState?.combatWindowKey)||isRtTrapLocked(u,"heal"))return false;
...
async function playCardOn(x,y,target){if(isBattleEnded())return setHint("La batalla ya terminó.");const card=selectedCard;if(!card)return;if(isPvpStep6fLimitedMode()&&!isPvpStep6hAllowedHandCard(card))return setHint("Paso 6H: solo puedes resolver UNIDADES y Fireball. Las demás cartas siguen bloqueadas.");if((privateState.honor||0)<effectiveCardCost(card,myPlayer))return setHint(`No tienes ${getResourceLabel(myPlayer)} suficiente.`);let units=[...(publicState.units||[])];if(card.type==="unit"){if(!summonZones(myPlayer).includes(`${x},${y}`))return setHint("Casilla inválida para invocación junto al líder.");const summonCostInfo=getCardCostBreakdown(card,myPlayer,units);const paidCostText=getPaidSummonCostText(card,myPlayer,units);let newUnit=makeUnit({...card,summonOrigin:"hand",fieldGeneratedSummon:false},x,y);if(ownerHasUnit(myPlayer===1?2:1,"yi_sun_sin",units)){newUnit={...newUnit,tempDexDebuff:(newUnit.tempDexDebuff||0)+4,tempGuardBuff:(newUnit.tempGuardBuff||0)-4,yiSunDebuffed:true};}units.push(newUnit);const lionFearSummon=applyAfricanLionFearAura(units);units=lionFearSummon.units;if(!(await commitCardPlay(card,{units,statusFxEvent:lionFearSummon.statusFxEvent||null,floatFxEvent:lionFearSummon.floatFxEvent||null},summonCostInfo.effective,[`J${myPlayer} invoca ${card.name} y ${paidCostText}.${isPvpStep6fLimitedMode()?" Paso 6H: queda en campo; MOV, DEF y ATTK están disponibles. Fireball también está habilitado. EFFECT sigue bloqueado.":" Puede moverse, defender o atacar inmediatamente."}${newUnit.yiSunDebuffed?" Bloqueo Naval: entra con -4 DX y -4 Guardia temporalmente.":""}`,...lionFearSummon.logs].join(" "))))return setHint("No se pudo confirmar la jugada. El estado no fue aplicado parcialmente.");setHint(`${card.name} fue invocada por ${summonCostInfo.effective} ${getResourceLabel(myPlayer)}.${summonCostInfo.sabotageStacks>0?` Sabotaje añadió +${summonCostInfo.sabotagePenalty}.`:""}${isPvpStep6fLimitedMode()?" Paso 6H: puede usar MOV, DEF o ATTK. Fireball está habilitado; EFFECT sigue bloqueado.":" Puede actuar inmediatamente."}`);if(newUnit.key==="hattori_hanzo")setHint("Hattori Hanzō ingresa con Sigilo. Contrato del Shogun se activará automáticamente contra la primera unidad enemiga que ataque desde Sigilo.");}else if(card.type==="equipment"){if(!target||!canEquipCardToUnit(card,target,myPlayer,units))return setHint(`Elige una unidad compatible con ${getEquipmentLeaderLabel(card)}.`);units=units.map(u=>u.id===target.id?equipCardOnUnit(card,u):u);if(!(await commitCardPlay(card,{units,floatFxEvent:makeFloatFxEvent("buff",units.find(u=>u.id===target.id)||target,0,{iconText:card.icon||"✦",labelText:"EQUIPO"})},null,`J${myPlayer} equipa ${card.name} a ${target.name}.`)))return setHint("No se pudo confirmar la jugada. El estado no fue aplicado parcialmente.");setHint(`${target.name} ahora lleva ${card.name}.`);}else if(card.spell==="damage"){const spellMagicKind=getCardMagicDamageType(card);const enemyRemainsAtCell=getUndeadRemainsAt(x,y,publicState).filter(r=>Number(r.owner)!==Number(myPlayer));if(!target&&enemyRemainsAtCell.length&&(spellMagicKind==="fire"||spellMagicKind==="ice")){const remainsHit=applyElementToUndeadRemains(publicState.undeadRemains||[],x,y,spellMagicKind,{enemyOwner:myPlayer===1?2:1});const bone=enemyRemainsAtCell[0];const pseudo={id:bone.id,owner:bone.owner,name:bone.name,x:bone.x,y:bone.y,rarity:"Legendaria"};tryPlaySound(spellMagicKind==="fire"?"attack_fire_magic":"spell_damage",.8);const label=spellMagicKind==="fire"?"INCINERADO":"CONGELADO +2";if(!(await commitCardPlay(card,{undeadRemains:remainsHit.remains,statusFxEvent:makeStatusFxEvent(spellMagicKind==="fire"?"burn_apply":"freeze_apply",pseudo,0),floatFxEvent:makeFloatFxEvent("info",pseudo,0,{iconText:spellMagicKind==="fire"?"🔥":"❄️",labelText:label})},null,`J${myPlayer} usa ${card.name} sobre Restos Persistentes. ${remainsHit.logs.join(" ")}`)))return setHint("No se pudo confirmar la jugada.");setHint(remainsHit.logs.join(" "));return;}if(!target||target.owner===myPlayer||!canDirectlyTarget(card,target))return setHint("Elige un objetivo rival visible o Restos Persistentes enemigos. Las unidades con Sigilo no pueden ser objetivo directo.");tryPlaySound(card.key==="fireball"?"attack_fire_magic":"spell_damage",card.key==="fireball"?.86:.72);const spellDamage=Math.max(0,Number(effectiveCardValue(card,"damage")||0));const fireAffinity=getUnitElementalAffinity(target,"fire");const appliesBurn=card.key==="fireball"&&!target.leader&&fireAffinity>0;const appliesSandSlow=card.key==="bolt"&&!target.leader;const sandSlowAmount=Math.max(0,Number(card.slowPermanent||0));const spellFxCaster=getLeader(myPlayer)||units.find(u=>u.owner===myPlayer&&u.leader);const spellBattleFxEvent=spellFxCaster?makeMagicFxEvent(spellFxCaster,target,spellMagicKind,{type:"spell",spellKey:card.key,effectAction:"damage",skipLaunchSound:true,impactScale:card.key==="fireball"?1.12:1,hit:true}):null;const beforeSpellDamage=[...units];let actualSpellDamage=spellDamage,spellAffinityMultiplier=1;units=units.map(u=>{if(u.id!==target.id)return u;const magicDamage=applyMagicHpDamage(u,spellDamage,spellMagicKind);actualSpellDamage=magicDamage.damage;spellAffinityMultiplier=magicDamage.multiplier;return magicDamage.unit;});units=applyLegendaryFatalSaves(units,[target.id]);const fatalSaveTriggered=!!units.find(u=>u.id===target.id&&u.hp>0)&&Number(target.hp||0)-actualSpellDamage<=0;units=units.map(u=>{if(u.id!==target.id||u.hp<=0)return u;let next=u;if(appliesBurn)next=applyBurnToUnit(next,card.name,card.burnTurns||2,card.burnDamage||1);if(appliesSandSlow)next={...next,mov:Math.max(0,Number(next.mov||0)-sandSlowAmount)};return next;}).filter(u=>u.hp>0);const spellLeaderMasteryResult=spellFxCaster&&!units.some(u=>u.id===target.id)?registerLocalLeaderMasteryKill(spellFxCaster,target):null;units=spellFxCaster?applyUnitMasteryRankUpToUnits(units,spellFxCaster,spellLeaderMasteryResult):units;const bloodVictoryResult=applyBloodVictoryForDeaths(beforeSpellDamage,units);units=bloodVictoryResult.units;const fireIncinerated=spellMagicKind==="fire"&&isUndeadUnit(target)&&!units.some(u=>u.id===target.id);const remainsElementHit=(spellMagicKind==="fire"||spellMagicKind==="ice")?applyElementToUndeadRemains(publicState.undeadRemains||[],x,y,spellMagicKind):{remains:publicState.undeadRemains||[],logs:[],changed:false};const affinityText=spellAffinityMultiplier===0?" · INMUNE al elemento":spellAffinityMultiplier>1?` · DEBILIDAD elemental ×${spellAffinityMultiplier}`:spellAffinityMultiplier<1?` · RESISTENCIA elemental ×${spellAffinityMultiplier}`:"";const actionLog=`J${myPlayer} usa ${card.name}: ${target.name} recibe ${actualSpellDamage} daño mágico directo${affinityText}${target.key==="honey_badger"?" tras Armadura Natural":""}${fatalSaveTriggered?". Último Aliento evita la derrota":""}${appliesBurn&&units.some(u=>u.id===target.id)?" y queda con Quemadura: +1 daño directo cada 10 s durante 20 s":""}${appliesSandSlow&&units.some(u=>u.id===target.id)?` y pierde -${sandSlowAmount} MOV permanente`:""}.${bloodVictoryResult.logs.length?` ${bloodVictoryResult.logs.join(" ")}`:""}${fireIncinerated?` INCINERADO: ${target.name} no deja restos capaces de reanimarse.`:""}${remainsElementHit.logs.length?` ${remainsElementHit.logs.join(" ")}`:""}${unitMasteryRankUpText(spellLeaderMasteryResult)}`;if(!(await commitCardPlay(card,{units,undeadRemains:remainsElementHit.remains,_undeadIncineratedIds:fireIncinerated?[target.id]:[],_clockKillCreditOwner:myPlayer,battleFxEvent:spellBattleFxEvent,floatFxEvent:makeFloatFxEvent("damage", units.find(u=>u.id===target.id)||target,actualSpellDamage),statusFxEvent:appliesBurn?makeStatusFxEvent("burn_apply", units.find(u=>u.id===target.id)||target,1):(card.key==="fireball"&&target.leader?makeStatusFxEvent("fire_impact", units.find(u=>u.id===target.id)||target,0):(appliesSandSlow?makeStatusFxEvent("debuff", units.find(u=>u.id===target.id)||target, sandSlowAmount):null))},null,actionLog)))return setHint("No se pudo confirmar la jugada. El estado no fue aplicado parcialmente.");await finalizeBattle(units,actionLog)}else if(card.spell==="paralysis"){if(!target||target.owner===myPlayer||target.leader||!canDirectlyTarget(card,target))return setHint("Elige una invocación rival visible. Parálisis no afecta líderes ni unidades con Sigilo no reveladas.");tryPlaySound("spell_cast",.72);units=units.map(u=>u.id===target.id?applyBasicParalysisSpell(u,card.name,publicState):u);const liveTarget=units.find(u=>u.id===target.id)||target;const spellFxCaster=getLeader(myPlayer)||units.find(u=>u.owner===myPlayer&&u.leader);const spellBattleFxEvent=spellFxCaster?makeMagicFxEvent(spellFxCaster,liveTarget,"lightning",{type:"spell",spellKey:card.key,effectAction:"paralysis",skipLaunchSound:true,impactSound:"impact_magic",hit:true}):null;if(!(await commitCardPlay(card,{units,battleFxEvent:spellBattleFxEvent,statusFxEvent:makeStatusFxEvent("paralysis_apply",liveTarget,0),floatFxEvent:makeFloatFxEvent("paralysis",liveTarget,0,{iconText:"⚡",labelText:"PARÁLISIS"})},null,`J${myPlayer} usa ${card.name}: ${target.name} queda paralizada temporalmente y no podrá moverse, atacar, defender ni contraatacar.`)))return setHint("No se pudo confirmar la jugada. El estado no fue aplicado parcialmente.");setHint(`${target.name} quedó paralizada temporalmente.`)}else if(card.spell==="poison"){if(!target||target.owner===myPlayer||target.leader||!canDirectlyTarget(card,target))return setHint("Elige una invocación rival visible. Veneno no afecta líderes ni unidades con Sigilo no reveladas.");if(isPoisonImmuneUnit(target))return setHint(`${target.name} es inmune al Veneno.`);tryPlaySound("spell_cast",.70);units=units.map(u=>u.id===target.id?applyBasicPoisonSpell(u,card.name,card.poisonTurns||3,card.poisonDamage||1):u);const liveTarget=units.find(u=>u.id===target.id)||target;const spellFxCaster=getLeader(myPlayer)||units.find(u=>u.owner===myPlayer&&u.leader);const spellBattleFxEvent=spellFxCaster?makeMagicFxEvent(spellFxCaster,liveTarget,"arcane",{type:"spell",spellKey:card.key,effectAction:"poison",skipLaunchSound:true,impactSound:"impact_magic",hit:true}):null;if(!(await commitCardPlay(card,{units,battleFxEvent:spellBattleFxEvent,statusFxEvent:makeStatusFxEvent("poison_apply",liveTarget,liveTarget.poisonDamage||1),floatFxEvent:makeFloatFxEvent("poison",liveTarget,liveTarget.poisonDamage||1,{iconText:"☠"})},null,`J${myPlayer} usa ${card.name}: ${target.name} recibe Veneno durante ${(card.poisonTurns||3)*10} s. El daño inicia en ${card.poisonDamage||1} y se duplica en cada tick.`)))return setHint("No se pudo confirmar la jugada. El estado no fue aplicado parcialmente.");setHint(`${target.name} quedó envenenada.`)}else if(card.spell==="buff"){if(!target||target.owner!==myPlayer)return setHint("Elige una unidad aliada.");tryPlaySound("spell_cast",.66);const bhTrap=resolveBuffHealLegendaryTraps(target,"buff",units);units=bhTrap.cancel?bhTrap.units:units.map(u=>u.id===target.id?{...u,buffAtk:(u.buffAtk||0)+effectiveCardValue(card,"buff")}:u);if(!(await commitCardPlay(card,{units,legendaryTraps:bhTrap.traps,floatFxEvent:bhTrap.floatFxEvent||(bhTrap.cancel?null:makeFloatFxEvent("buff", units.find(u=>u.id===target.id)||target,effectiveCardValue(card,"buff"),{iconText:"▲"})),statusFxEvent:bhTrap.statusFxEvent||null},null,bhTrap.cancel?bhTrap.logs.join(" "):`J${myPlayer} usa ${card.name}: ${target.name} gana +${effectiveCardValue(card,"buff")} AT temporalmente.`)))return setHint("No se pudo confirmar la jugada. El estado no fue aplicado parcialmente.")}else if(card.spell==="shield"){if(!target||target.owner!==myPlayer)return setHint("Elige una unidad aliada.");tryPlaySound("spell_cast",.66);const bhTrap=resolveBuffHealLegendaryTraps(target,"Guardia/buff",units);units=bhTrap.cancel?bhTrap.units:units.map(u=>u.id===target.id?{...u,tempGuardBuff:(u.tempGuardBuff||0)+effectiveCardValue(card,"guard")}:u);if(!(await commitCardPlay(card,{units,legendaryTraps:bhTrap.traps,floatFxEvent:bhTrap.floatFxEvent||(bhTrap.cancel?null:makeFloatFxEvent("guard_buff", units.find(u=>u.id===target.id)||target,effectiveCardValue(card,"guard"),{iconText:"🛡"})),statusFxEvent:bhTrap.statusFxEvent||null},null,bhTrap.cancel?bhTrap.logs.join(" "):`J${myPlayer} usa ${card.name}: ${target.name} gana +${effectiveCardValue(card,"guard")} GUARDIA durante hasta 10 s.`)))return setHint("No se pudo confirmar la jugada. El estado no fue aplicado parcialmente.")}else if(card.trap==="guard"){if(!target||target.owner!==myPlayer)return setHint("Elige una unidad aliada.");tryPlaySound("trap_trigger",.66);const bhTrap=resolveBuffHealLegendaryTraps(target,"Guardia/buff",units);units=bhTrap.cancel?bhTrap.units:units.map(u=>u.id===target.id?{...u,warningRuneGuard:effectiveCardValue(card,"guard"),warningRuneCardName:card.name}:u);if(!(await commitCardPlay(card,{units,legendaryTraps:bhTrap.traps,floatFxEvent:bhTrap.floatFxEvent||(bhTrap.cancel?null:makeFloatFxEvent("guard_buff", units.find(u=>u.id===target.id)||target,effectiveCardValue(card,"guard"),{iconText:"◆",labelText:"RUNA"})),statusFxEvent:bhTrap.statusFxEvent||null},null,bhTrap.cancel?bhTrap.logs.join(" "):`J${myPlayer} coloca ${card.name} sobre ${target.name}. La próxima vez que sea atacada obtendrá +${effectiveCardValue(card,"guard")} GUARDIA durante ese combate.`)))return setHint("No se pudo confirmar la jugada. El estado no fue aplicado parcialmente.")}else if(card.spell==="heal"){if(!target||target.owner!==myPlayer)return setHint(cardCleanseEnabled(card)?"Elige una unidad aliada herida o con estado curable.":"Elige una unidad aliada herida.");if(!canReceiveHealFromCard(card,target,myPlayer))return setHint(cardCleanseEnabled(card)?"Esa unidad no está herida ni tiene estados curables.":"Esa unidad no está herida.");if(target.noHealWindowKey===publicState.combatWindowKey||isRtTrapLocked(target,"heal")||target.noHealWhilePoisoned)return setHint(`${target.name} no puede curarse ahora.`);const healAmount=effectiveCardValue(card,"heal");const canCleanse=cardCleanseEnabled(card);const hadCurableStatus=canCleanse&&hasCurableStatus(target);const actualHeal=Math.max(0,Math.min(effectiveMaxHp(target),(target.hp||0)+healAmount)-(target.hp||0));const bhTrap=resolveBuffHealLegendaryTraps(target,"curación",units);const healFxCaster=getLeader(myPlayer)||units.find(u=>u.owner===myPlayer&&u.leader);const healBattleFxEvent=!bhTrap.cancel&&healFxCaster?makeMagicFxEvent(healFxCaster,target,"heal",{type:"heal",spellKey:card.key,effectAction:canCleanse?"cleanse":"heal",skipLaunchSound:true,impactSound:"heal",hit:true}):null;units=bhTrap.cancel?bhTrap.units:units.map(u=>u.id===target.id?(canCleanse?clearCurableStatuses({...u,hp:Math.min(effectiveMaxHp(u),(u.hp||0)+healAmount)}):{...u,hp:Math.min(effectiveMaxHp(u),(u.hp||0)+healAmount)}):u);if(!(await commitCardPlay(card,{units,legendaryTraps:bhTrap.traps,battleFxEvent:healBattleFxEvent,floatFxEvent:bhTrap.floatFxEvent||(bhTrap.cancel?null:makeFloatFxEvent("heal", units.find(u=>u.id===target.id)||target,actualHeal,{iconText:"✚",labelText:hadCurableStatus&&actualHeal<=0?"LIMPIA":""})),statusFxEvent:bhTrap.statusFxEvent||null},null,bhTrap.cancel?bhTrap.logs.join(" "):`J${myPlayer} usa ${card.name}: ${target.name} ${actualHeal>0?`cura ${actualHeal} HP`:"no recupera HP"}${hadCurableStatus?" y limpia Sangrado/Veneno/Quemadura":""}.`)))return setHint("No se pudo confirmar la jugada. El estado no fue aplicado parcialmente.")}else if(card.trap==="beast_cell"){if(target)return setHint("Elige una celda libre para la trampa.");if(getCellBeastTrapAt(x,y))return setHint("Ya hay una trampa de cacería en esa celda.");tryPlaySound("trap_trigger",.68);if(!(await commitCardPlay(card,{beastTraps:[...getBeastTraps(),makeBeastTrap(card,myPlayer,x,y)]},null,`J${myPlayer} coloca ${card.name} en una celda de cacería.`)))return setHint("No se pudo confirmar la jugada. El estado no fue aplicado parcialmente.")}else if(card.trap==="beast_target"){if(!target||target.owner===myPlayer||target.leader||!canTargetStealth(card,target))return setHint("Elige una invocación rival válida.");const leader=getLeader(myPlayer)||{x:0,y:0};if(dist(leader,target)>3)return setHint("Objetivo fuera de rango 3 del líder.");tryPlaySound("trap_trigger",.70);const trapMs=getTrapTimedDurationMs(target,"minor");units=units.map(u=>u.id===target.id?withRtTrapDebuff(u,"agi",2,trapMs,card.name):u);if(!(await commitCardPlay(card,{units,floatFxEvent:makeFloatFxEvent("debuff", units.find(u=>u.id===target.id)||target,2,{iconText:"▼"})},null,`J${myPlayer} usa ${card.name}: ${target.name} pierde -2 AGI durante ${Math.round(trapMs/1000)} s.`)))return setHint("No se pudo confirmar la jugada. El estado no fue aplicado parcialmente.")}else if(card.trap==="reveal_stealth"){tryPlaySound("trap_trigger",.70);const rev=revealStealthInRadius(units,myPlayer,{x,y},card.radius||2,card.name);const stage8Detection=typeof makeStage8StealthDetectionEvent==="function"?makeStage8StealthDetectionEvent(myPlayer,{x,y},card.radius||2,card.name):null;const revealLog=stage8Detection?`J${myPlayer} usa ${card.name}: explora el área en busca de unidades con Sigilo.`:`J${myPlayer} usa ${card.name}: revela ${rev.count} unidad${rev.count===1?"":"es"} con Sigilo en el área.`;if(!(await commitCardPlay(card,{units:rev.units,stealthDetectionEvent:stage8Detection||null},null,revealLog)))return setHint("No se pudo confirmar la jugada. El estado no fue aplicado parcialmente.")}else if(card.trap==="slow"){if(!target||target.owner===myPlayer||target.leader||!canTargetStealth(card,target))return setHint("Elige una invocación rival.");tryPlaySound("trap_trigger",.70);const slowAmount=effectiveCardValue(card,"slow");const agiSlow=Number(card.agiSlow||0);const trapMs=getTrapTimedDurationMs(target,slowAmount>=4?"major":slowAmount>=2?"medium":"minor");units=units.map(u=>{
        if(u.id!==target.id)return u;
        let next=withRtTrapDebuff(u,"mov",slowAmount,trapMs,card.name);
        if(agiSlow>0)next=withRtTrapDebuff(next,"agi",agiSlow,trapMs,card.name);
        return next;
      });if(!(await commitCardPlay(card,{units,floatFxEvent:makeFloatFxEvent("debuff", units.find(u=>u.id===target.id)||target,slowAmount,{iconText:"▼"})},null,`J${myPlayer} activa ${card.name}: ${target.name} pierde ${slowAmount} MOV${agiSlow>0?` y ${agiSlow} AGI`:""} durante ${Math.round(trapMs/1000)} s. DET mostrará el origen del debuff.`)))return setHint("No se pudo confirmar la jugada. El estado no fue aplicado parcialmente.")}else if(card.trap==="legendary_mark"){if(!canMarkWithLegendaryTrap(card,target))return setHint("Ese objetivo no cumple las condiciones de esta Trampa Legendaria.");tryPlaySound("trap_trigger",.74);const trap=makeTrapMark(card,target,myPlayer);if(!(await commitCardPlay(card,{legendaryTraps:[...getActiveLegendaryTraps(),trap]},null,`J${myPlayer} coloca ${card.name} sobre ${target.name} (${getUnitTrapTierLabel(target)}). La trampa esperará su condición.`)))return setHint("No se pudo confirmar la jugada. El estado no fue aplicado parcialmente.");setHint(`${target.name} quedó marcado por ${card.name}.`)}clearSelection()}
